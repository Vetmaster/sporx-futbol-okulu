...
begin
  ...
  if student_ids is null or present_student_ids is null
     ...
     or cardinality(student_ids) <> (
       select count(distinct listed_student.student_id)
       from unnest(student_ids) as listed_student(student_id)
     )
     or exists (
       select 1
       from unnest(present_student_ids) as present_student(student_id)
       where present_student.student_id is null
          or present_student.student_id <> all(student_ids)
     )
     or exists (
       select 1
       from unnest(student_ids) as listed_student(student_id)
       left join public.students student on student.id = listed_student.student_id
       where listed_student.student_id is null
  ...
  returning attendance_sessions.id into saved_session_id;

  insert into public.attendance_records (session_id, student_id, present)
  select
    saved_session_id,
    listed_student.student_id,
    listed_student.student_id = any(present_student_ids)
  from unnest(student_ids) as listed_student(student_id)
