-- Ensure newly approved schools cannot use the app before starting trial or payment.
...
begin
  ...
  if application_row.status = 'APPROVED' and application_row.approved_school_id is not null then
    ...
    limit 1;
    return query select application_row.approved_school_id, created_onboarding.id;
    return;
  end if;
